...
function wrap2(alg, key, cek, iv) {
  return __async(this, null, function* () {
    ...
    const { ciphertext: encryptedKey, tag } = yield encrypt_default(jweAlgorithm, cek, key, iv, new Uint8Array(0));
...
function flattenedDecrypt(jwe, key, options) {
  return __async(this, null, function* () {
    ...
    let plaintext = yield decrypt_default(enc, cek, ciphertext, iv, tag, additionalData);
...
var FlattenedEncrypt = class {
  ...
  encrypt(key, options) {
    return __async(this, null, function* () {
      ...
      const { ciphertext, tag } = yield encrypt_default(enc, this._plaintext, cek, this._iv, additionalData);
      const jwe = {
